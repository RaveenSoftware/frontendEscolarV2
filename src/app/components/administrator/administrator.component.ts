...
@Component({
  ...
  template: `
    <div class="administrator-management">
      ...
      <div *ngIf="showForm" class="administrator-form">
        <h2>{{ editingAdministrator ? 'Editar' : 'Agregar' }} Administrador</h2>
        <form [formGroup]="form" (ngSubmit)="saveAdministrator()">
        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let administrator of administrators">
              <td>{{ administrator.codigoInstitucional }}</td>
              <td>{{ administrator.nombre }}</td>
              <td>{{ administrator.numeroDocumento }}</td>
              <td>{{ getFacultadNombre(administrator.facultadId) }}</td>
              <td>
                <span [class]="'status-badge ' + (administrator.estado ? 'active' : 'inactive')">
                  {{ administrator.estado ? 'Activo' : 'Inactivo' }}
                </span>
              </td>
              <td class="actions">
                <button class="btn-edit" (click)="editAdministrator(administrator)">
                  <i class="fas fa-edit"></i>
                </button>
                <button class="btn-delete" (click)="deleteAdministrator(administrator.id!)">
...
export class AdministratorManagementComponent implements OnInit {
  ...
  roles: any[] = [];
  showForm = false;
  editingAdministrator: Administrator | null = null;
  ...
  editAdministrator(administrator: Administrator) {
    this.showForm = true;
    this.editingAdministrator = administrator;
    this.form.patchValue({
      ...administrator,
      fechaNacimiento: administrator.fechaNacimiento?.split('T')[0]
    });
  }
  ...
  saveAdministrator() {
    ...
    const administratorData = this.form.value;

    if (this.editingAdministrator) {
      this.administratorService.update(this.editingAdministrator.id!, administratorData).subscribe({
        next: () => {
      ...
      this.administratorService.create(administratorData).subscribe({
        next: () => {
